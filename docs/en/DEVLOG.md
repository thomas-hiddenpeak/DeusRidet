...
- [2026-06-01](devlog/2026-06-01.md) — Vires: broaden Foreground heartbeats to machina prefill/decode + ASR (`738fc66`); accuracy 93.5% → 93.6% (+0.1 pp) on `tests/test.mp3`
- [2026-05-30](devlog/2026-05-30.md) — DiariZen-v2 native CUDA pipeline became the sole/default path: `DEUSRIDET_DIARIZEN_NATIVE` was folded into default-on `DEUSRIDET_DIARIZEN_ENABLE` (opt out with `=0`), the Python-IPC bridge was retired, live accuracy held at 93.6–93.7%, and finalize RTF improved to ~0.10/0.099
- [2026-05-29](devlog/2026-05-29.md) — DiariZen-v2 Hybrid pipeline P0/P1/P2 shipped via C++/Python IPC fast-path: facade bridge, live wiring + WS finalize broadcast, periodic + on-demand recluster with transcript holdback, WebUI recluster panel + 12-diarizen RFC
- [2026-05-27](devlog/2026-05-27.md) — Phase 3c: first semantic (eyes-on, no scripted scoring) read of the production diarisation path on `tests/test.mp3` first 600s — pattern → evidence → suspected cause → candidate intervention
- [2026-05-26](devlog/2026-05-26.md) — Phase 14: reliability-weighted affinity inside the reclusterer (`w_i=min(1,dur_i/1.5)`); s1800 +1.03 pp but full_60m −3.56 pp — third consecutive window-local lever rejected, `window_sec=180` ceiling holds
- [2026-05-25](devlog/2026-05-25.md) — Phase 12: overlap-aware embedding A/B (negative) — splicing the RMS-dominant separated channel back into source PCM before CAM++/WavLM re-extraction hurts both windows
- [2026-05-23](devlog/2026-05-23.md) — Step 19c + 19d breakthrough: MULTI-GATE CAM++ sliding cosine probe before FULL identify (multi-speaker VAD → read-only `peek_best`) + coverage fix on the 4-speaker Mandarin replay corpus
