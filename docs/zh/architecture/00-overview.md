...
### 第 12 步 —— 意识引导外提完成（2026-04-24 关闭）

`awaken.cpp` 内 126 行的 LLM + 意识流引导块横跨四个子系统
（machina + memoria + conscientia + persona），单子系统外观不合身。
改以平级 Actus TU 的形式外提：

- `src/actus/awaken_consciousness.{h,cpp}` —— 声明
  `struct ConscientiaBootstrap`（Tokenizer、ModelWeights、
  InferenceState、CacheManager、ConscientiStream、PersonaConfig、
  bool loaded）与 `bootstrap_consciousness()`；成功返回 0
  （无论是否加载 LLM），任一致命步骤失败返回 1，并在原地回撤已分配的部分资源。
- `src/actus/awaken.cpp` 由 392 行缩到 268 行。
  六个未使用的 `*_ptr` 影子局部已删除；下游引用整体改写为 `cb.<成员>`。
  `awaken()` 在文件末尾仍负责对应的 destroy/free 配对。

提交 `767ebe5` 核验：构建干净、awaken HTTP=200 WS=101。

后续如浮现新的 Nexus / Memoria / Persona / Orator 反向渗透，
则自然触发下一个外观。
