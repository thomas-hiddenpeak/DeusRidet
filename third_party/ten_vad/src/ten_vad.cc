...
// DeusRidet extension: create with configurable model path.
int ten_vad_create2(ten_vad_handle_t* handle, size_t hop_size,
                    float threshold, const char* model_path) {
  if (AUP_Aed_create(handle) < 0) {
    return -1;
  }
  Aed_St* stHdl = (Aed_St*)(*handle);
  ...
  // Set model path (default if NULL).
  if (model_path) {
    snprintf(stHdl->modelPath, sizeof(stHdl->modelPath), "%s", model_path);
  } else {
    snprintf(stHdl->modelPath, sizeof(stHdl->modelPath),
...
int ten_vad_create(ten_vad_handle_t* handle, size_t hop_size, float threshold) {
  return ten_vad_create2(handle, hop_size, threshold, nullptr);
}
