#!/usr/bin/env bash
# Step 7: Test reclusterer on the new tighter baseline (31.0% raw).
# Usage: tools/run_recluster_on.sh <cell_name>
set -euo pipefail
...
nohup env DEUSRIDET_RECLUSTERER_ENABLE=1 "$BINARY" awaken > "$AWAKEN_LOG" 2>&1 &
PID=$!; disown $PID
...
{"cell":"$CELL","matrix":"v1","commit":"$COMMIT","binary_sha256":"$BIN_SHA","env":{"DEUSRIDET_RECLUSTERER_ENABLE":"1"},"fixed":{"audio":"tests/test.mp3","speed":2.0,"max_sec":3615},"start_ts":"$START_TS","end_ts":"$END_TS","wall_sec":$WALL,"replay_exit":$REPLAY_EXIT,"ring_buffer_overflows":$OVF}
JSON
echo "=== cell $CELL done ==="
