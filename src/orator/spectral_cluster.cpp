 ...
 * @philosophical_role Peer TU of spectral_cluster.h carrying the full clustering
 *         algorithm (PCA → similarity → p-pruning → normalised Laplacian →
 *         eigengap K-selection → K-means++ → temporal smoothing → centroid
 *         merge). Split out of the header under R1 because the 560-line `inline`
 *         definition kept spectral_cluster.h at 626 lines — over the 500-line
 *         hard cap for headers — and every translation unit that included it
 *         dragged all 560 lines through its parser.
