...
ASRResult ASREngine::transcribe(const float* samples, int num_samples,
    ...
    // 14. Hallucination detection — mark Whisper-specific subtitle artifacts.
    // Only detect patterns that can NEVER be real speech (subtitle overlays,
    // subscribe prompts). Short filler words (嗯, 对, 好的, 谢谢) are NOT
    // filtered here — they are legitimate speech. Pre-VAD filtering is the
    // proper defense against noise-induced hallucinations.
    // The hallucinated flag is advisory — the pipeline decides whether to
    // suppress based on user-configurable toggle.
    {
        bool is_hallucination = false;

        // Whisper subtitle artifacts — these phrases are generated by the
        // model's subtitle training data, never from real microphone input.
        static const char* whisper_artifacts[] = {
            "谢谢观看", "感谢观看", "谢谢大家", "感谢收看",
            "谢谢收看", "请订阅", "字幕", "字幕由",
            "Thank you for watching", "Thanks for watching",
            "Subtitles by", "Subscribe", "Please subscribe",
        };
        for (const auto* p : whisper_artifacts) {
            if (text == p) { is_hallucination = true; break; }
        }

        if (is_hallucination) {
            result.hallucinated = true;
            // Text is preserved — pipeline controls suppression.
        }
    }
