/**
 * @file src/sensus/auditus/speaker_tracker_check_stages.cpp
 * @philosophical_role
 *   Step 11 A2 — heavy branch bodies of `SpeakerTracker::check()`.
 *   Five stages with verbatim bodies from the pre-split monolith
 *   (commit c77efde): identify-new, compute-signals, handle-change,
 *   handle-overlap, handle-continue. Peer TU of speaker_tracker_check.cpp.
 * @serves
...
// ============================================================
// Stage 4: identify from scratch (no current speaker).
// ============================================================
void SpeakerTracker::check_identify_new_(const std::vector<float>& emb,
                                         float f0, int n, bool is_fast_path) {
...
// ============================================================
// Stage 5a: compute change/overlap signals (sim/EMA/OD).
// ============================================================
void SpeakerTracker::track_compute_signals_(const std::vector<float>& emb,
                                            const std::vector<float>& pcm_f32,
                                            int n, float jitter,
                                            float& sim_to_ref,
...
// ============================================================
// Stage 5b: speaker change branch.
// ============================================================
void SpeakerTracker::track_handle_change_(const std::vector<float>& emb,
                                          float f0, int n, float sim_to_ref) {
...
// ============================================================
// Stage 5c: overlap branch — MossFormer2 separation + gating.
// ============================================================
void SpeakerTracker::track_handle_overlap_(std::vector<float>& pcm_f32,
    ...
    if (!(separator_.initialized() && enable_separator_.load(std::memory_order_relaxed)))
        return;
    if (n < 3200) return;  // minimum 200ms for meaningful separation
    ...
    (void)pcm_f32;  // OD path consumed it upstream; separator uses raw.
...
// ============================================================
// Stage 5d: same-speaker continuation branch.
// ============================================================
void SpeakerTracker::track_handle_continue_(const std::vector<float>& emb,
                                            float f0, float sim_to_ref) {
