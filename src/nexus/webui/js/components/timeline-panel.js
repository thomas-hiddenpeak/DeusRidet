...
// 3 independent lanes per time row: VAD, ASR, Speaker.
...
const ROW_H = LH.time + LH.vad + LH.asr + LH.spk + LH.gap;
...
export class TimelinePanel {
    constructor() {
        ...
        this.spkSegments  = [];  // {start, end, spkId, spkName, spkSim, spkConf, spkSrc}
        this.maxEntries    = 800;
    ...
    onTranscript(obj) {
        ...
        if (this.spkSegments.length > this.maxEntries) this.spkSegments.shift();
    ...
    _drawRow(c, W, y, tStart, tEnd, secsPerRow) {
        ...
        const lanes = [
            ...
            { name: 'SPK', y: tyTop + LH.time + LH.vad + LH.asr,    h: LH.spk },
        ];
        ...
        for (const seg of this.spkSegments) {
            if (seg.end < tStart || seg.start > tEnd) continue;
            const px1 = Math.max(x0, tx(Math.max(seg.start, tStart)));
            const px2 = Math.min(W, tx(Math.min(seg.end, tEnd)));
            if (px2 - px1 < 1) continue;
    ...
    _onHover(e) {
        ...
        else if (localY >= spkTop && localY < spkTop + LH.spk) lane = 'spk';
        ...
        if (lane === 'vad') {
            ...
            const seg = this.spkSegments.find(s => s.start <= timeSec && s.end >= timeSec);
            if (seg) {
                const name = seg.spkName || (seg.spkId >= 0 ? `S${seg.spkId}` : '?');
                html = `<strong>SPK</strong> ${seg.start.toFixed(1)}s – ${seg.end.toFixed(1)}s<br>`
